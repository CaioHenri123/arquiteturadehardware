...
;Dada as equações, resolva-as utilizando assembler, e através do registro SREG, responda se o valor se o resultado é positivo, negativo, ou se houve estouro de variável. 
;Salve o status do SREG de cada operação em um GPR register (r0 a r31). 

;Observação 1) O código deverá ser entregue através do GitHub

;Observação 2) No código do programa, as equações deverão ser resolvidas uma seguida da outra, utilize registradores diferentes para armazenar os resultados das equações(r16,r17,r18 e etc...).
; Não se esqueça de limpar as flags do SREG antes de começar a resolução das equações na sequência.

;Observação 3) Sim, o TP vale 12, isso da a oportunidade de você poder errar uma questão e ainda acertar 100% do TP.
